class Pantallas {
  ...
  // Pantalla de videojuego (aca trabajamos agregar el minijuego)
  /*void videojuegos(float x, float y) {
   stageSettings();
   image(imagenes[3], x, y);
   textsettings(30, 430, 730, 70, 10);
  ...
  void dormir(float x, float y) {
    ...
    if (sleep_time > 120) {
      ...
      fill(255);
      text("Levantarse", width/4.53, height/1.2);
      text("Seguir\ndurmiento", width/1.37, height/1.22);
